...
class LUT {
  public:
    ...
    static constexpr float SCALE = static_cast<float>(SIZE - 1) / X_RANGE;
    static constexpr float INV_SCALE = X_RANGE / static_cast<float>(SIZE - 1);

    LUT() {
        for (int i = 0; i < SIZE; ++i) {
            table_[i] = std::exp(X_MIN + i * INV_SCALE);
        }
    }

    float operator()(float x) const {
        if (x <= X_MIN) return table_[0];
        if (x >= X_MAX) return table_[SIZE - 1];
...
inline const LUT& get_lut() {
    static LUT lut;
    return lut;
}

inline float lut(float x) { return get_lut()(x); }
...
struct DiodeIV_LUT {
    static void eval(float v, float& i, float& g) {
        using namespace bjt;
        if (v > V_CRIT) {
            float exp_crit = exp_impl::lut(V_CRIT * V_T_INV);
            g = I_S * V_T_INV * exp_crit;
            i = I_S * (exp_crit - 1.0f) + g * (v - V_CRIT);
        } else if (v < -10.0f * V_T) {
            i = -I_S;
            ...
            float exp_v = exp_impl::lut(v * V_T_INV);
            i = I_S * (exp_v - 1.0f);
            g = I_S * V_T_INV * exp_v + 1e-12f;
        }
    }
...
struct DiodeIV_Pade22 {
    static void eval(float v, float& i, float& g) {
        using namespace bjt;
        if (v > V_CRIT) {
            float exp_crit = exp_impl::pade22(V_CRIT * V_T_INV);
            g = I_S * V_T_INV * exp_crit;
            i = I_S * (exp_crit - 1.0f) + g * (v - V_CRIT);
        } else if (v < -10.0f * V_T) {
            i = -I_S;
            ...
            float exp_v = exp_impl::pade22(v * V_T_INV);
            i = I_S * (exp_v - 1.0f);
            g = I_S * V_T_INV * exp_v + 1e-12f;
        }
    }
...
struct DiodeIV_Pade33 {
    static void eval(float v, float& i, float& g) {
        using namespace bjt;
        if (v > V_CRIT) {
            float exp_crit = exp_impl::pade33(V_CRIT * V_T_INV);
            g = I_S * V_T_INV * exp_crit;
            i = I_S * (exp_crit - 1.0f) + g * (v - V_CRIT);
        } else if (v < -10.0f * V_T) {
            i = -I_S;
            ...
            float exp_v = exp_impl::pade33(v * V_T_INV);
            i = I_S * (exp_v - 1.0f);
            g = I_S * V_T_INV * exp_v + 1e-12f;
        }
    }
...
// WaveShaperクラステンプレート
// =============================================================================
template <typename DiodeIV, int Iterations, typename CoeffSource>
class WaveShaper {
  public:
    WaveShaper()
...
// 各種exp近似バリエーション
using WaveShaperLUT = WaveShaper<DiodeIV_LUT, 2, OwnCoeffs>;
using WaveShaperPade = WaveShaper<DiodeIV_Pade22, 2, OwnCoeffs>;
using WaveShaperPade33 = WaveShaper<DiodeIV_Pade33, 2, OwnCoeffs>;
...
using WaveShaperLUT = tb303::WaveShaperLUT;
using WaveShaperPade = tb303::WaveShaperPade;
using WaveShaperPade33 = tb303::WaveShaperPade33;
