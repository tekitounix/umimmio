...
| 14 | security | 80% | SHA-256 を umi/crypto/ に移動 | |
| 17 | shell | 95% | 軽微な差分のみ | |
...
**ファイル:** `lib/umi/port/mcu/usb_midi_input.hh` (新規), `lib/umi/port/mcu/uart_midi_input.hh` (新規)
...
**ファイル:** `lib/umi/port/common/common/dwt.hh` (新規)
...
**ファイル:** `lib/umi/kernel/metrics.hh` に追加
...
**ファイル:** `lib/umi/kernel/umi_monitor.hh` に追加
...
**ファイル:** `lib/umi/kernel/protection.hh` (新規)
...
既存: SHA-512 + Ed25519 は `lib/umi/crypto/`。SHA-256 は `lib/umiboot/include/umiboot/auth.hh` 内にソフトウェア実装あり。CRC32 は umiboot に実装済み。

#### Step 4-1: SHA-256 を umi/crypto/ に移動

**ファイル:** `lib/umi/crypto/sha256.hh` (新規), `lib/umi/crypto/sha256.cc` (新規)

umiboot/auth.hh 内の `detail::Sha256` を独立モジュールとして抽出。umiboot 側は umi/crypto/sha256.hh を include するように変更。
...
**ファイル:** `lib/umi/kernel/system_task.hh` (新規)
...
**ファイル:** `lib/umi/kernel/updater.hh` (新規)
...
- `lib/umi/port/mcu/internal_flash.hh` — STM32F4 内蔵 Flash
- `lib/umi/port/mcu/spi_flash.hh` — W25Qxx SPI Flash
- `lib/umi/port/mcu/sd_spi.hh` — SD カード (SPI モード)
