...
> - バッファアクセスは `input(ch)` / `output(ch)` メソッドを提供（nullptr チェック込み）
...
void process(umi::AudioContext& ctx) {
    ...
    float* out_l = ctx.output(0);
    float* out_r = ctx.output(1);
    if (!out_l) return;
...
存在しないチャンネルへのアクセスは `ctx.input(ch)` / `ctx.output(ch)` が `nullptr` を返す。
