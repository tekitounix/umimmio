...
- `tests/compile_fail/value_typesafe.cc`: compile-fail ガード — 非 Numeric フィールドでの `value()` が拒否される
...
4. **compile-fail ガード** — 不正操作がコンパイルされないことを確認（WO 読み出し、RO 書き込み、安全フィールドでの `value()`）
...
- 全ホストテストパス
- compile-fail 契約テストパス (read_wo, write_ro, value_typesafe)
