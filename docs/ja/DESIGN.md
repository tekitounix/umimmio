...
| 型 | 用途 |
|------|---------|
| `Region<addr, T>` | 固定アドレスにストレージ型 T を持つレジスタ |
| `Field<Region, offset, width>` | Region 内のビットフィールド |
| `Value<Field, val>` | Field の名前付き定数 |
| `DynamicValue<Field>` | Field のランタイム値 |
...
using EN   = umi::mmio::Field<CTRL, 0, 1>;   // ビット0 の 1 ビットフィールド
using MODE = umi::mmio::Field<CTRL, 1, 2>;   // ビット1-2 の 2 ビットフィールド
```
...
- `DynamicValue<Region, T>`: 遅延範囲チェック付きランタイム値。
