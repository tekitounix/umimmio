...
`AppHeader` は [lib/umi/kernel/app_header.hh](lib/umi/kernel/app_header.hh) と同一でなければならない。
...
## 3. エントリポイント契約
エントリポイントは `lib/umi/app/crt0.cc` の `_start()` を前提とする。
...
### 5.1 syscall（ABI 準拠）
syscall の番号と呼び出し規約は [lib/umi/app/syscall.hh](lib/umi/app/syscall.hh) に一致しなければならない。
...
現行実装の例は [lib/umi/kernel/loader.hh](lib/umi/kernel/loader.hh) の `SharedMemory` を参照する。
