...
**現状分析:**
- `lib/umi/port/platform/embedded/platform/syscall.hh` — **既にr12規約** (r12=nr, r0-r3=args)
- `lib/umi/app/syscall.hh` — **旧r0規約** (r0=nr, r1-r4=args)
...
**変更ファイル:**
1. `lib/umi/app/syscall.hh` — `call()` をr12規約に変更
...
4. `lib/umi/app/umi_app.hh` — `register_processor()` 等のsyscall呼出を確認
...
**変更ファイル:**
- `lib/umi/core/shared_state.hh` (新規作成)
- `lib/umi/kernel/loader.hh` (SharedMemory更新)
- `examples/stm32f4_kernel/src/kernel.cc` (params読み書き箇所修正)
- `examples/synth_app/src/main.cc` (SharedMemory参照修正)

**実装内容:**
```cpp
// lib/umi/core/shared_state.hh
...
**変更ファイル:**
- `lib/umi/core/audio_context.hh` (フィールド追加)
...
#### Phase 3a: データ構造定義
**新規作成:** `lib/umi/core/event_router.hh`
...
**変更:** `lib/umi/core/event_router.hh`
...
**新規作成:** `lib/umi/core/param_mapping.hh`
...
**変更ファイル:**
- `lib/umi/app/syscall.hh` (nr::set_app_config=20 ~ nr::send_param_request=25 追加)
- `lib/umi/kernel/syscall/syscall_numbers.hh` (カーネル側番号追加)
...
**新規作成:** `lib/umi/core/control_event.hh`
...
**変更:** `lib/umi/app/umi_app.hh` に Controller用API追加
