...
- [ ] consumer側（examples, kernel）の `#include` パスを新形式に段階的移行
...
- [ ] `lib/umimmio/` ディレクトリ作成
- [ ] `lib/umimmio/xmake.lua` 作成（`umi.mmio` ターゲット）
- [ ] `lib/umimmio/include/mmio/mmio.hh` — コアヘッダ移植
- [ ] `lib/umimmio/include/mmio/transport/direct.hh` — DirectTransport
- [ ] `lib/umimmio/include/mmio/transport/i2c.hh` — I2cTransport
- [ ] `lib/umimmio/test/xmake.lua` 作成
- [ ] `lib/umimmio/test/test_mmio.cc` — 基本テスト移植
- [ ] `lib/umimmio/test/test_transport.hh` — モックTransport移植
- [ ] `lib/umi/xmake.lua` に `umi.mmio` ターゲット追加

### HAL Concepts 配置 (umi_mmio hal/ → umiport/concepts/)

- [ ] `concepts/result.hh` — `Result<T, ErrorCode>` 共通エラー型
- [ ] `concepts/gpio.hh` — GpioPin, GpioPort
- [ ] `concepts/i2c.hh` — I2cMaster
- [ ] `concepts/uart.hh` — Uart
- [ ] `concepts/timer.hh` — Timer, DelayTimer, PwmTimer
- [ ] `concepts/i2s.hh` — I2sMaster
- [ ] `concepts/interrupt.hh` — InterruptController
- [ ] `concepts/audio.hh` — AudioDevice
...
- [ ] `device/` ディレクトリ構造を作成
- [ ] `device/cs43l22/cs43l22_regs.hh` — mmio Register定義（STM32F4-Disco用コーデック）
- [ ] `device/cs43l22/cs43l22.hh` — AudioCodec concept実装
- [ ] 既存の `board/stm32f4_disco/board/cs43l22.hh` をdevice/に移行

### テスト・検証

**L1 (ホスト単体):**
- [ ] `test_mmio.cc` — mmioコアのホストテスト通過
- [ ] `test_concept_compliance.cc` — 全Conceptのstatic_assert検証
- [ ] `test_device_codec.cc` — CS43L22ドライバのモックI2Cテスト
- [ ] `xmake test` パス

**完了条件**: umimmioテスト通過 + Concept充足テスト通過 + 既存テスト全パス
