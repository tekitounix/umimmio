...
class Adapter {
  public:
    explicit Adapter(Proc& processor) noexcept : processor_(processor) {}
    ...
    [[nodiscard]] float audio_load() const noexcept { return audio_load_; }
  ...
  private:
    void audio_task_entry() {
        ...
        while (true) {
            ...
            AudioContext ctx{.inputs = std::span<const sample_t* const>(
                                 const_cast<const sample_t**>(input_ptrs_.data()), Config.num_inputs),
    ...
    void update_load(uint32_t cycles) {
        ...
        constexpr uint32_t cycles_per_buffer = cpu_freq / (Config.sample_rate / Config.buffer_size);
