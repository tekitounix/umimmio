...
public:
    ...
    int init() noexcept {
        ...
        slim_cfg_.read = [](const umi::fs::SlimConfig* cfg, uint32_t block, uint32_t off, void* buf,
                            uint32_t size) -> int {
            auto* self = static_cast<StorageService*>(cfg->context);
            return self->flash_.read(block, off, buf, size);
        };
        slim_cfg_.prog = [](const umi::fs::SlimConfig* cfg, uint32_t block, uint32_t off, const void* buf,
                            uint32_t size) -> int {
            auto* self = static_cast<StorageService*>(cfg->context);
    ...
    int32_t consume_result() noexcept {
        if (!result_slot_.ready) return fs_errno::EAGAIN;
...
private:
    ...
    static int32_t fat_to_errno(umi::fs::FatResult r) noexcept {
        ...
        switch (r) {
            case FR::OK: return 0;
            case FR::DISK_ERR: return fs_errno::EIO;
            case FR::NO_FILE: return fs_errno::ENOENT;
            case FR::NO_PATH: return fs_errno::ENOENT;
            case FR::DENIED: return fs_errno::EISDIR;
            case FR::EXIST: return fs_errno::EEXIST;
            case FR::WRITE_PROTECTED: return fs_errno::EIO;
            case FR::NOT_READY: return fs_errno::ENODEV;
            case FR::INVALID_NAME: return fs_errno::EINVAL;
            case FR::INVALID_PARAMETER: return fs_errno::EINVAL;
            default: return fs_errno::EIO;
        }
    }
    ...
    int32_t dispatch(const FsRequest& req) noexcept {
        ...
        switch (req.syscall_nr) {
            // File operations
            case nr::file_open:     return do_file_open(req);
            case nr::file_read:     return do_file_read(req);
            case nr::file_write:    return do_file_write(req);
            case nr::file_close:    return do_file_close(req);
            case nr::file_seek:     return do_file_seek(req);
            case nr::file_tell:     return do_file_tell(req);
            case nr::file_size:     return do_file_size(req);
            case nr::file_truncate: return do_file_truncate(req);
            case nr::file_sync:     return do_file_sync(req);
            // Directory operations
            case nr::dir_open:      return do_dir_open(req);
            case nr::dir_read:      return do_dir_read(req);
            case nr::dir_close:     return do_dir_close(req);
            case nr::dir_seek:      return do_dir_seek(req);
            case nr::dir_tell:      return do_dir_tell(req);
            // Path operations
            case nr::stat:          return do_stat(req);
            case nr::fstat:         return do_fstat(req);
            case nr::mkdir:         return do_mkdir(req);
            case nr::remove:        return do_remove(req);
            case nr::rename:        return do_rename(req);
            // Custom attributes
            case nr::getattr:       return do_getattr(req);
            case nr::setattr:       return do_setattr(req);
            case nr::removeattr:    return do_removeattr(req);
            // FS info
            case nr::fs_stat:       return do_fs_stat(req);
            default: return fs_errno::EINVAL;
        }
    }
    ...
    int32_t do_file_open(const FsRequest& req) noexcept {
        ...
        if (is_flash_path(path)) {
            if (!flash_mounted_) return fs_errno::ENODEV;
            int fd = alloc_fd(FdType::SLIM_FILE);
            if (fd < 0) return fd;
            int rc = slim_.file_open(slim_files_[fd], strip_prefix(path), flags);
            if (rc != 0) { fd_type_[fd] = FdType::NONE; return rc; }
            return fd;
        } else {
            if (!sd_mounted_) return fs_errno::ENODEV;
            int fd = alloc_fd(FdType::FAT_FILE);
            if (fd < 0) return fd;
            // Convert OpenFlags to FATfs mode
            uint8_t mode = 0x01; // FA_READ
            auto f = static_cast<uint32_t>(flags);
            if (f & 0x02) mode = 0x02; // FA_WRITE
            if (f & 0x03) mode = 0x03; // FA_READ | FA_WRITE
            if (f & 0x0100) mode |= 0x08; // FA_CREATE_NEW → FA_OPEN_ALWAYS
            if (f & 0x0400) mode |= 0x04; // FA_CREATE_ALWAYS (TRUNC)
    ...
    int32_t do_file_read(const FsRequest& req) noexcept {
        ...
        if (fd < 0 || fd >= static_cast<int>(MAX_OPEN_FILES)) return fs_errno::EBADF;
        if (fd_type_[fd] == FdType::SLIM_FILE) {
            ...
            if (rc != umi::fs::FatResult::OK) return fat_to_errno(rc);
    ...
    int32_t do_dir_open(const FsRequest& req) noexcept {
        ...
        if (is_flash_path(path)) {
            ...
            if (rc != 0) { dir_type_[dfd] = FdType::NONE; return rc; }
            return dfd;
        } else {
            if (!sd_mounted_) return fs_errno::ENODEV;
    ...
    int32_t do_dir_read(const FsRequest& req) noexcept {
        ...
        if (dfd < 0 || dfd >= static_cast<int>(MAX_OPEN_DIRS)) return fs_errno::EBADF;
    ...
    int32_t do_getattr(const FsRequest& req) noexcept {
        ...
        if (!is_flash_path(path)) return fs_errno::ENOSYS;
        if (!flash_mounted_) return fs_errno::ENODEV;
