...
static void format_fat16_image(uint8_t* storage) {
    ...
    bs[14] = 1;
    bs[15] = 0; // reserved sectors = 1
    bs[16] = 2; // number of FATs
    bs[17] = 0x00;
    bs[18] = 0x02; // root entries = 512
    bs[19] = 0x00;
    bs[20] = 0x02; // total sectors = 512
    bs[21] = 0xF8; // media type
    bs[22] = 1;
    bs[23] = 0; // sectors per FAT = 1
    bs[24] = 0x3F;
    bs[25] = 0;
    bs[26] = 0xFF;
    bs[27] = 0;
...
inline int s_sync(const umi::fs::SlimConfig*) {
    return 0;
}
} // namespace slim_detail
...
struct SlimAdapter {
    ...
    int write_file(const char* path, const void* data, uint32_t size) {
        umi::fs::SlimFile f{};
        int err = fs.file_open(
            f, path, umi::fs::SlimOpenFlags::WRONLY | umi::fs::SlimOpenFlags::CREAT | umi::fs::SlimOpenFlags::TRUNC);
        if (err)
            return err;
    ...
    int read_file(const char* path, void* buf, uint32_t buf_size, uint32_t* out_size) {
        ...
        if (err < 0)
            return err;
        if (out_size)
            *out_size = static_cast<uint32_t>(err);
        return 0;
    }
