...
inline void init_pll3() {
    ...
    transport.modify(RCC::PLLCFGR::PLL3VCOSEL::Reset{}, // Wide VCO
    ...
    transport.write(RCC::PLL3DIVR::DIVN3::value(294),
...
inline void init_sai_gpio() {
    ...
    for (auto pin : pins) {
        gpio_configure_pin<GPIOE>(
            transport, pin, gpio_mode::ALTERNATE, gpio_otype::PUSH_PULL, gpio_speed::MEDIUM, gpio_pupd::PULL_UP);
...
inline void init_sai() {
    ...
    transport.write(SAI1_A::CR1::MODE::value(sai_mode::MASTER_TX),
    ...
    transport.write(SAI1_A::CR2::FTH::value(sai_fth::EMPTY));

    // Frame: 64-bit frame (2 × 32-bit slots for 24-bit MSB-justified)
    // FRL = 64-1 = 63, FSALL = 32-1 = 31
    transport.write(SAI1_A::FRCR::FRL::value(63),
    ...
    transport.write(SAI1_A::SLOTR::FBOFF::value(0),
    ...
    transport.write(SAI1_B::CR1::MODE::value(sai_mode::SLAVE_RX),
    ...
    transport.write(SAI1_B::CR2::FTH::value(sai_fth::EMPTY));

    // Same frame config as Block A (64-bit frame for 24-bit MSB-justified)
    transport.write(SAI1_B::FRCR::FRL::value(63),
    ...
    transport.write(SAI1_B::SLOTR::FBOFF::value(0),
...
inline void init_audio_dma(std::int32_t* tx_buf, std::int32_t* rx_buf, std::uint32_t buf_size) {
    ...
    // Configure stream
    transport.write(DMA1_Stream0::CR::DIR::value(dma_dir::MEM_TO_PERIPH),
    ...
    transport.write(DMA1_Stream1::CR::DIR::value(dma_dir::PERIPH_TO_MEM),
...
inline void init_i2c4() {
    ...
    for (std::uint8_t pin : {SeedBoard::i2c_scl_pin, SeedBoard::i2c_sda_pin}) {
        gpio_configure_pin<GPIOB>(
            transport, pin, gpio_mode::ALTERNATE, gpio_otype::OPEN_DRAIN, gpio_speed::MEDIUM, gpio_pupd::PULL_UP);
...
inline void i2c4_write_reg(std::uint8_t dev_addr, std::uint8_t reg, std::uint8_t data) {
    ...
    transport.write(I2C4::CR2::value((static_cast<std::uint32_t>(dev_addr) << 1) | // SADD (7-bit left-shifted)
                                     (2U << 16) |                                  // NBYTES=2
                                     (1U << 25) |                                  // AUTOEND
                                     (1U << 13)                                    // START
                                     ));
...
inline std::uint8_t i2c4_read_reg(std::uint8_t dev_addr, std::uint8_t reg) {
    ...
    transport.write(I2C4::CR2::value((static_cast<std::uint32_t>(dev_addr) << 1) | (1U << 16) | // NBYTES=1
                                     (1U << 13)                                                 // START
                                     ));
...
inline void i2c4_wm8731_write(std::uint8_t reg, std::uint16_t data) {
    ...
    transport.write(I2C4::CR2::value((static_cast<std::uint32_t>(device::WM8731::i2c_address) << 1) |
                                     (2U << 16) | // NBYTES=2
                                     (1U << 25) | // AUTOEND
                                     (1U << 13)   // START
                                     ));
...
inline void init_codec(BoardVersion version) {
    ...
    case BoardVersion::SEED_REV4: {
        ...
        gpio_configure_pin<GPIOB>(transport,
                                  SeedBoard::codec_reset_pin,
                                  gpio_mode::OUTPUT,
    ...
    case BoardVersion::SEED_REV5: {
        ...
        init_i2c4();
        auto wm_write = [](std::uint8_t reg, std::uint16_t data) { i2c4_wm8731_write(reg, data); };
