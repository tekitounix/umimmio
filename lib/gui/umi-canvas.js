...
const UmiCanvas = (function() {
    ...
    return {
        ...
        render() {
            if (wasmModule && wasmModule._upf_gui_render) {
                wasmModule._upf_gui_render();
            }
        },
        ...
        hitTest(x, y) {
            if (wasmModule && wasmModule._upf_gui_hit_test) {
                return wasmModule._upf_gui_hit_test(x, y);
            }
            return -1;
        },
        
        drag(paramIndex, deltaY) {
            if (wasmModule && wasmModule._upf_gui_drag) {
                wasmModule._upf_gui_drag(paramIndex, deltaY);
            }
        },
        
        click(paramIndex) {
            if (wasmModule && wasmModule._upf_gui_click) {
                wasmModule._upf_gui_click(paramIndex);
            }
        }
    };
})();
