...
コアペリフェラルは **メモリマップド (MMIO)** であり、umimmio の型システムで表現可能。
本カテゴリは MMIO アクセスのコアペリフェラルのみを対象とする。

**非 MMIO のコアレジスタ・命令** (ARM MSR/MRS レジスタ、RISC-V CSR、Xtensa Special Registers、
メモリバリア、排他アクセス、DSP/SIMD 命令等) は
**[C2: コア特殊レジスタ・命令イントリンシクス](C02_CORE_INTRINSICS.md)** を参照。
...
### 3.1 CSR (Control and Status Registers) — → C2 参照

CSR (mstatus, mie, mtvec, mcause 等) は専用命令 (`csrr`/`csrw`) でアクセスする非 MMIO レジスタ。
詳細は **[C2: コアイントリンシクス §3.1](C02_CORE_INTRINSICS.md#31-csr-control-and-status-registers)** を参照。
...
### 4.1 Special Registers / Window Registers — → C2 参照

Xtensa の Special Registers (PS, EPC, CCOUNT, INTENABLE 等) および Window Registers は
`rsr`/`wsr`/`xsr` 命令でアクセスする非 MMIO レジスタ。
詳細は **[C2: コアイントリンシクス §4.1–4.2](C02_CORE_INTRINSICS.md#41-special-registers-rsrwsrxsr-アクセス)** を参照。

### 4.2 Interrupt Matrix — ESP32 固有 MMIO
...
### 5.4 RISC-V CSR — → C2 参照

CSR の constexpr メタデータとアクセスヘルパーの コード例は
**[C2: コアイントリンシクス §5.2](C02_CORE_INTRINSICS.md#52-risc-v-csr-アクセス)** を参照。
