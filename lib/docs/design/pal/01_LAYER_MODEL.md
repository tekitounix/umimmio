...
| アーキテクチャ | 含まれる定義 | 例 |
|-------------|------------|-----|
| ARM Cortex-M | コア例外テーブル構造、NVIC/SCB/SysTick レジスタ、MSR/MRS レジスタ、バリア命令 | Reset, NMI, HardFault, SysTick_Handler, PRIMASK, DSB/ISB |
| RISC-V | 標準 CSR、mtime/mtimecmp、FENCE 命令 | mstatus, mie, mtvec, mepc, FENCE |
| Xtensa | Special Registers、Window Registers、同期命令 | PS, EPC, CCOUNT, MEMW, WAITI |
...
| カテゴリ | L1 | L2 | L3 | L4 |
|---------|----|----|----|----|
| C1 コアペリフェラルレジスタ (MMIO) | NVIC, SCB, SysTick | MPU, FPU, DWT, ITM, SAU | — | — |
| C2 コアイントリンシクス (非MMIO) | バリア, 割り込み制御, WFI/WFE | FPSCR, DSP/SIMD, TZ, キャッシュ | — | — |
| C3 コアシステム定義 | — | FPU/MPU/DSP/TZ 有無 | — | NVIC 優先度ビット数 |
| C4 割り込みベクター | コア例外 | — | デバイス固有 IRQ | テーブルサイズ |
| C5 メモリマップ | — | — | メモリ種類 | base/size |
| C6 ペリフェラルレジスタ | — | — | レジスタ構造 | インスタンス有無 |
| C7 GPIO ピンマルチプレクシング | — | — | MUX 方式 | AF テーブル |
| C8 クロックツリー | — | — | PLL/バス構造 | 最大周波数 |
| C9 DMA マッピング | — | — | DMA モデル | チャネル割当 |
| C10 電力管理 | — | — | スリープモード | ドメイン構成 |
| C11 セキュリティ | — | SAU/PMP | Flash 保護 | OTP/eFuse |
| C12 デバイスメタデータ | — | — | — | 全項目 |
| C13 リンカ / スタートアップ | C ランタイム初期化 | セクション配置 | Boot 方式 | メモリサイズ |
| C14 デバッグ / トレース | CoreDebug | ITM, DWT, ETM | デバッグI/F | — |
