# 04. ボード設定アーキテクチャ — 設定の統一と局所化

**ステータス:** 設計中
**関連文書:**
- [04a_BOARD_CONFIG_ANALYSIS.md](04a_BOARD_CONFIG_ANALYSIS.md) — 全フレームワーク横断比較と根拠付け
...
この「BSP = 設定値の集合体」という捉え方は、調査した全ての成熟したフレームワーク（Zephyr, modm, Mbed OS, Rust embedded, CMSIS-Pack）で確認された共通原則である（→ [04a_BOARD_CONFIG_ANALYSIS.md §9.7](04a_BOARD_CONFIG_ANALYSIS.md)）。
...
全フレームワークの横断調査から、**「完全統一スキーマ」はどのフレームワークも採用していない**ことが確認された（→ [04a_BOARD_CONFIG_ANALYSIS.md §8.3](04a_BOARD_CONFIG_ANALYSIS.md)）。UMI は「共通スキーマ + ファミリー固有オプショナルセクション」方式を採用する。
...
| 文書 | 内容 |
|------|------|
| [04a_BOARD_CONFIG_ANALYSIS.md](04a_BOARD_CONFIG_ANALYSIS.md) | 全フレームワーク横断比較。6 類型分類、7 設計決定の根拠付け |
