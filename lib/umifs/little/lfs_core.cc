...
static bool lfs_mlist_isopen(LfsMlist *head,
        LfsMlist *node) {
...
static void lfs_mlist_remove(lfs_t *lfs, LfsMlist *mlist) {
...
static void lfs_mlist_append(lfs_t *lfs, LfsMlist *mlist) {
...
static lfs_stag_t lfs_dir_getslice(lfs_t *lfs, const lfs_mdir_t *dir,
    ...
    while (off >= sizeof(lfs_tag_t) + lfs_tag_dsize(ntag)) {
        ...
        lfs_tag_t tag = ntag;
        int err = lfs_bd_read(lfs,
                nullptr, &lfs->rcache, sizeof(ntag),
...
static int lfs_dir_traverse_filter(void *p,
    ...
    lfs_tag_t *filtertag = static_cast<lfs_tag_t*>(p);
    (void)buffer;
...
popped:
        ...
        if (lfs_tag_type3(tag) == LFS_FROM_NOOP) {
            ...
            dir = static_cast<decltype(dir)>(buffer);
            off = 0;
            ptag = 0xffffffff;
            attrs = nullptr;
            attrcount = 0;
            ...
            for (unsigned i = 0; i < lfs_tag_size(tag); i++) {
                const LfsAttr *a = static_cast<const LfsAttr*>(buffer);
...
static lfs_stag_t lfs_dir_fetchmatch(lfs_t *lfs,
    ...
    for (int i = 0; i < 2; i++) {
        int err = lfs_bd_read(lfs,
                nullptr, &lfs->rcache, sizeof(revs[i]),
    ...
    for (int i = 0; i < 2; i++) {
        ...
        while (true) {
            ...
            off += lfs_tag_dsize(ptag);
            int err = lfs_bd_read(lfs,
                    nullptr, &lfs->rcache, lfs->cfg->block_size,
                    dir->pair[0], off, &tag, sizeof(tag));
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
                    // can't continue?
...
static int lfs_dir_fetch(lfs_t *lfs,
    ...
    return (int)lfs_dir_fetchmatch(lfs, dir, pair,
            (lfs_tag_t)-1, (lfs_tag_t)-1, nullptr, nullptr, nullptr);
}
...
static int lfs_dir_find_match(void *data,
        lfs_tag_t tag, const void *buffer) {
    struct lfs_dir_find_match *name = static_cast<struct lfs_dir_find_match*>(data);
    lfs_t *lfs = name->lfs;
    const struct lfs_diskoff *disk = static_cast<const struct lfs_diskoff*>(buffer);
...
static int lfs_dir_commitattr(lfs_t *lfs, struct lfs_commit *commit,
    ...
    if (!(tag & 0x80000000)) {
        ...
        for (lfs_off_t i = 0; i < dsize-sizeof(tag); i++) {
            ...
            uint8_t dat;
            err = lfs_bd_read(lfs,
                    nullptr, &lfs->rcache, dsize-sizeof(tag)-i,
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit) {
    ...
    while (commit->off < end) {
        ...
        if (noff >= end && noff <= lfs->cfg->block_size - lfs->cfg->prog_size) {
            ...
            int err = lfs_bd_read(lfs,
                    nullptr, &lfs->rcache, lfs->cfg->prog_size,
    ...
    int err = lfs_bd_crc(lfs,
            nullptr, &lfs->rcache, off1+sizeof(uint32_t),
    ...
    err = lfs_bd_crc(lfs,
            nullptr, &lfs->rcache, sizeof(uint32_t),
...
static int lfs_dir_alloc(lfs_t *lfs, lfs_mdir_t *dir) {
    ...
    int err = lfs_bd_read(lfs,
            nullptr, &lfs->rcache, sizeof(dir->rev),
...
static int lfs_dir_commit_size(void *p, lfs_tag_t tag, const void *buffer) {
    lfs_size_t *size = static_cast<lfs_size_t*>(p);
    (void)buffer;
...
static int lfs_dir_commit_commit(void *p, lfs_tag_t tag, const void *buffer) {
    struct lfs_dir_commit_commit *commit = static_cast<struct lfs_dir_commit_commit*>(p);
...
fixmlist:;
    ...
    for (LfsMlist *d = lfs->mlist; d; d = d->next) {
        ...
        if (tag != LFS_ERR_NOENT) {
            ...
            state = lfs_dir_relocatingcommit(lfs, &pdir, ppair, LFS_MKATTRS(
                        {LFS_MKTAG_IF(moveid != 0x3ff,
                            LFS_TYPE_DELETE, moveid, 0), nullptr},
                        {tag, ldir.pair}),
                    nullptr);
...
static int lfs_mkdir_(lfs_t *lfs, const char *path) {
    ...
    err = lfs_dir_commit(lfs, &cwd.m, LFS_MKATTRS(
            {LFS_MKTAG(LFS_TYPE_CREATE, id, 0), nullptr},
...
static int lfs_dir_open_(lfs_t *lfs, lfs_dir_t *dir, const char *path) {
    lfs_stag_t tag = lfs_dir_find(lfs, &dir->m, &path, nullptr);
    if (tag < 0) {
        return tag;
    }
    ...
    lfs_mlist_append(lfs, (LfsMlist *)dir);
...
static int lfs_dir_close_(lfs_t *lfs, lfs_dir_t *dir) {
    ...
    lfs_mlist_remove(lfs, (LfsMlist *)dir);

    return 0;
}

static int lfs_dir_read_(lfs_t *lfs, lfs_dir_t *dir, LfsInfo *info) {
...
static int lfs_ctz_find(lfs_t *lfs,
    ...
    lfs_off_t current = lfs_ctz_index(lfs, LFS_SCALAR_TMP(lfs_off_t, size-1));
...
static int lfs_ctz_traverse(lfs_t *lfs,
    ...
    lfs_off_t index = lfs_ctz_index(lfs, LFS_SCALAR_TMP(lfs_off_t, size-1));
...
static int lfs_file_opencfg_(lfs_t *lfs, lfs_file_t *file,
    ...
    lfs_mlist_append(lfs, (LfsMlist *)file);
    ...
    if (tag == LFS_ERR_NOENT) {
        ...
        err = lfs_dir_commit(lfs, &file->m, LFS_MKATTRS(
                {LFS_MKTAG(LFS_TYPE_CREATE, file->id, 0), nullptr},
                {LFS_MKTAG(LFS_TYPE_REG, file->id, nlen), path},
                {LFS_MKTAG(LFS_TYPE_INLINESTRUCT, file->id, 0), nullptr}));
    ...
    if (file->cfg->buffer) {
        file->cache.buffer = static_cast<uint8_t*>(file->cfg->buffer);
    } else {
        file->cache.buffer = static_cast<uint8_t*>(lfs_malloc(lfs->cfg->cache_size));
...
static int lfs_file_open_(lfs_t *lfs, lfs_file_t *file,
        const char *path, int flags) {
    static const LfsFileConfig defaults = {0};
...
static int lfs_file_close_(lfs_t *lfs, lfs_file_t *file) {
    ...
    // remove from list of mdirs
    lfs_mlist_remove(lfs, (LfsMlist*)file);
...
static lfs_ssize_t lfs_file_flushedread(lfs_t *lfs, lfs_file_t *file,
    ...
    while (nsize > 0) {
        ...
        if (!(file->flags & LFS_F_READING) ||
            ...
            if (!(file->flags & LFS_F_INLINE)) {
                int err = lfs_ctz_find(lfs, nullptr, &file->cache,
        ...
        if (file->flags & LFS_F_INLINE) {
            int err = lfs_dir_getread(lfs, &file->m,
                    nullptr, &file->cache, lfs->cfg->block_size,
                    LFS_MKTAG(0xfff, 0x1ff, 0),
                    LFS_MKTAG(LFS_TYPE_INLINESTRUCT, file->id, 0),
                    file->off, data, diff);
            if (err) {
        ...
        if (!(file->flags & LFS_F_WRITING) ||
            ...
            if (!(file->flags & LFS_F_INLINE)) {
                if (!(file->flags & LFS_F_WRITING) && file->pos > 0) {
                    ...
                    int err = lfs_ctz_find(lfs, nullptr, &file->cache,
                            file->ctz.head, file->ctz.size,
                            file->pos-1, &file->block, LFS_SCALAR_TMP(lfs_off_t, 0));
                    if (err) {
                        file->flags |= LFS_F_ERRED;
                        return err;
                    }
...
static lfs_ssize_t lfs_file_write_(lfs_t *lfs, lfs_file_t *file,
    ...
    if (!(file->flags & LFS_F_WRITING) && file->pos > file->ctz.size) {
        ...
        while (file->pos < pos) {
            lfs_ssize_t res = lfs_file_flushedwrite(lfs, file, LFS_SCALAR_TMP(uint8_t, 0), 1);
            if (res < 0) {
                return res;
            }
        }
...
static lfs_soff_t lfs_file_seek_(lfs_t *lfs, lfs_file_t *file,
    ...
    if ((file->flags & LFS_F_READING)
            && file->off != lfs->cfg->block_size) {
        int oindex = lfs_ctz_index(lfs, LFS_SCALAR_TMP(lfs_off_t, file->pos));
        lfs_off_t noff = npos;
...
static int lfs_file_truncate_(lfs_t *lfs, lfs_file_t *file, lfs_off_t size) {
    ...
    if (size < oldsize) {
        ...
        if (size <= lfs->inline_max) {
            ...
            err = lfs_ctz_find(lfs, nullptr, &file->cache,
                    file->ctz.head, file->ctz.size,
                    size-1, &file->block, LFS_SCALAR_TMP(lfs_off_t, 0));
            if (err) {
                return err;
            }
        ...
        // fill with zeros
        while (file->pos < size) {
            res = lfs_file_write_(lfs, file, LFS_SCALAR_TMP(uint8_t, 0), 1);
            if (res < 0) {
                return (int)res;
            }
        }
...
static int lfs_stat_(lfs_t *lfs, const char *path, LfsInfo *info) {
    lfs_mdir_t cwd;
    lfs_stag_t tag = lfs_dir_find(lfs, &cwd, &path, nullptr);
    if (tag < 0) {
        return (int)tag;
    }
...
static int lfs_remove_(lfs_t *lfs, const char *path) {
    ...
    err = lfs_dir_commit(lfs, &cwd, LFS_MKATTRS(
            {LFS_MKTAG(LFS_TYPE_DELETE, lfs_tag_id(tag), 0), nullptr}));
    if (err) {
        lfs->mlist = dir.next;
        return err;
    }
...
static int lfs_rename_(lfs_t *lfs, const char *oldpath, const char *newpath) {
    ...
    // find old entry
    lfs_mdir_t oldcwd;
    lfs_stag_t oldtag = lfs_dir_find(lfs, &oldcwd, &oldpath, nullptr);
    ...
    err = lfs_dir_commit(lfs, &newcwd, LFS_MKATTRS(
            {LFS_MKTAG_IF(prevtag != LFS_ERR_NOENT,
                LFS_TYPE_DELETE, newid, 0), nullptr},
            {LFS_MKTAG(LFS_TYPE_CREATE, newid, 0), nullptr},
            {LFS_MKTAG(lfs_tag_type3(oldtag),
                newid, lfs_path_namelen(newpath)), newpath},
            {LFS_MKTAG(LFS_FROM_MOVE, newid, lfs_tag_id(oldtag)), &oldcwd},
            {LFS_MKTAG_IF(samepair,
                LFS_TYPE_DELETE, newoldid, 0), nullptr}));
    if (err) {
    ...
    if (!samepair && lfs_gstate_hasmove(&lfs->gstate)) {
        ...
        err = lfs_dir_commit(lfs, &oldcwd, LFS_MKATTRS(
                {LFS_MKTAG(LFS_TYPE_DELETE, lfs_tag_id(oldtag), 0), nullptr}));
        if (err) {
            lfs->mlist = prevdir.next;
            return err;
        }
...
static int lfs_removeattr_(lfs_t *lfs, const char *path, uint8_t type) {
    return lfs_commitattr(lfs, path, type, nullptr, 0x3ff);
}
#endif
...
static int lfs_init(lfs_t *lfs, const LfsConfig *cfg) {
    lfs->cfg = cfg;
    ...
    LFS_ASSERT(lfs->cfg->read != nullptr);
#ifndef LFS_READONLY
    LFS_ASSERT(lfs->cfg->prog != nullptr);
    LFS_ASSERT(lfs->cfg->erase != nullptr);
    LFS_ASSERT(lfs->cfg->sync != nullptr);
#endif
    ...
    if (lfs->cfg->read_buffer) {
        lfs->rcache.buffer = static_cast<uint8_t*>(lfs->cfg->read_buffer);
    } else {
        lfs->rcache.buffer = static_cast<uint8_t*>(lfs_malloc(lfs->cfg->cache_size));
    ...
    if (lfs->cfg->prog_buffer) {
        lfs->pcache.buffer = static_cast<uint8_t*>(lfs->cfg->prog_buffer);
    } else {
        lfs->pcache.buffer = static_cast<uint8_t*>(lfs_malloc(lfs->cfg->cache_size));
    ...
    if (lfs->cfg->lookahead_buffer) {
        lfs->lookahead.buffer = static_cast<uint8_t*>(lfs->cfg->lookahead_buffer);
    } else {
        lfs->lookahead.buffer = static_cast<uint8_t*>(lfs_malloc(lfs->cfg->lookahead_size));
...
#ifndef LFS_READONLY
static int lfs_format_(lfs_t *lfs, const LfsConfig *cfg) {
    int err = 0;
    {
        ...
        err = lfs_dir_commit(lfs, &root, LFS_MKATTRS(
                {LFS_MKTAG(LFS_TYPE_CREATE, 0, 0), nullptr},
        ...
        root.erased = false;
        err = lfs_dir_commit(lfs, &root, nullptr, 0);
        if (err) {
            goto cleanup;
        }
...
static int lfs_mount_(lfs_t *lfs, const LfsConfig *cfg) {
...
static int lfs_fs_stat_(lfs_t *lfs, LfsFsInfo *fsinfo) {
...
int lfs_fs_traverse_(lfs_t *lfs,
    ...
    while (!lfs_pair_isnull(dir.tail)) {
        ...
        for (uint16_t id = 0; id < dir.count; id++) {
            ...
            if (lfs_tag_type3(tag) == LFS_TYPE_CTZSTRUCT) {
                err = lfs_ctz_traverse(lfs, nullptr, &lfs->rcache,
...
static int lfs_fs_parent_match(void *data,
        lfs_tag_t tag, const void *buffer) {
    struct lfs_fs_parent_match *find = static_cast<struct lfs_fs_parent_match*>(data);
    lfs_t *lfs = find->lfs;
    const struct lfs_diskoff *disk = static_cast<const struct lfs_diskoff*>(buffer);
    (void)tag;
...
static int lfs_fs_demove(lfs_t *lfs) {
    ...
    err = lfs_dir_commit(lfs, &movedir, LFS_MKATTRS(
            {LFS_MKTAG(LFS_TYPE_DELETE, moveid, 0), nullptr}));
    if (err) {
        return err;
    }
    ...
    if (lfs->cfg->compact_thresh
        ...
        while (!lfs_pair_isnull(mdir.tail)) {
            ...
            if (!mdir.erased || ((lfs->cfg->compact_thresh == 0)
                ...
                mdir.erased = false;
                err = lfs_dir_commit(lfs, &mdir, nullptr, 0);
                if (err) {
                    return err;
                }
            }
...
/// v1 migration ///
static int lfs_migrate_(lfs_t *lfs, const LfsConfig *cfg) {
    struct lfs1 lfs1;
...

    {
        ...
        while (!lfs_pair_isnull(dir1.d.tail)) {
            ...
            err = lfs_dir_commit(lfs, &dir2, nullptr, 0);
            if (err) {
                goto cleanup;
            }
            ...
            while (true) {
                ...
                uint16_t id;
                err = lfs_dir_find(lfs, &dir2, LFS_SCALAR_TMP(const char*, name), &id);
