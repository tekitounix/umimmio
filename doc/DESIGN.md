...
マクロは使用しない。インクルードするファイルの差し替えで構成を切り替える。

```cpp
// 製品版（マイクロカーネル）
#include "config/microkernel.hh"

// 開発ボード（モノリシック）
#include "config/monolithic.hh"
```

各構成ファイルが同じインターフェースを提供し、アプリケーションコードは変更不要。

```
config/
├── microkernel.hh   # MPU有効、syscall経由
└── monolithic.hh    # MPU無効、直接呼び出し
```
