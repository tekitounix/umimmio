...
| **マクロ排除** | プリプロセッサマクロは使用しない。実装切り替えはインクルードパスで行う |
...
マクロは使用しない。ビルド時のインクルードパス切り替えで構成を選択。
...
```cpp
// アプリケーションコード（共通）
#include "umi_config.hh"  // ビルド設定によりどちらかが選択される
```
...
同名ファイル `umi_config.hh` が同じインターフェースを提供し、アプリケーションコードは変更不要。
...
### 9.2 ポート構成とインクルードパス切り替え

全ての実装切り替えはインクルードパスで行う。同名ファイルを異なるディレクトリに配置し、ビルド時に `add_includedirs()` で選択。

```
port/
├── arm/cortex-m/           # アーキテクチャ共通
│   └── cortex_m.hh         # DWT, NVIC, SCB, SysTick 統合
├── board/
...
│   └── stub/               # ホストテスト用
│       └── hw.hh           # スタブ実装（同名）
└── vendor/stm32/stm32f4/   # STM32 ペリフェラル
    └── peripheral.hh       # GPIO, RCC, UART 統合
