...
└─────────────────────────────────────────────────────────────┘
        ...
        ▼                     ▼                     ▼
┌───────────────┐     ┌───────────────┐     ┌───────────────┐
│  UMIM Adapter  │     │   Embedded    │     │  VST3/CLAP    │
...
```c
// ノートオンイベント
void umi_note_on(uint8_t note, uint8_t velocity);

// ノートオフイベント
void umi_note_off(uint8_t note);
```
...
// パラメータメタデータ
const char* umi_get_param_name(uint32_t index);    // パラメータ名
float umi_get_param_min(uint32_t index);           // 最小値
float umi_get_param_max(uint32_t index);           // 最大値
float umi_get_param_default(uint32_t index);       // デフォルト値
uint8_t umi_get_param_curve(uint32_t index);       // カーブ種別
const char* umi_get_param_unit(uint32_t index);    // 単位（Hz, dB等）
```
...
void umi_process_cc(uint8_t channel, uint8_t cc, uint8_t value);

// 動的MIDIラーン
void umi_midi_learn(uint8_t cc, uint32_t param_id);
void umi_midi_unlearn(uint8_t cc);
```
...
```c
// プラグインメタデータ
const char* umi_get_name(void);      // プラグイン名
const char* umi_get_vendor(void);    // ベンダー名
const char* umi_get_version(void);   // バージョン文字列
uint32_t umi_get_type(void);         // プラグイン種別
```

### マルチチャンネル対応

```c
uint32_t umi_get_input_count(void);   // 入力チャンネル数
uint32_t umi_get_output_count(void);  // 出力チャンネル数
void umi_process_multi(const float** inputs, float** outputs, 
...
```c
uint32_t umi_get_state_size(void);                        // 状態サイズ
void umi_get_state(uint8_t* buffer, uint32_t size);       // 状態を取得
void umi_set_state(const uint8_t* buffer, uint32_t size); // 状態を復元
```

### レイテンシ報告

```c
uint32_t umi_get_latency(void);  // サンプル単位
```

## パラメータカーブ

`umi_get_param_curve()` が返す値に対応するカーブタイプ:
...
2. **プラグイン内部**: プラグインが `umi_get_buffer_ptr()` でゼロコピー用バッファを提供
...
// エクスポートを取得
auto umi_create = wasm_runtime_lookup_function(instance, "umi_create");
auto umi_process = wasm_runtime_lookup_function(instance, "umi_process");

// 初期化
wasm_runtime_call(umi_create, sample_rate);

// オーディオ処理
wasm_runtime_call(umi_process, input_ptr, output_ptr, frames);
```
...
// リンクして実行
m3_FindFunction(&umi_process, runtime, "umi_process");
m3_Call(umi_process, input, output, frames);
```
...
function validateUMIM(exports) {
    const required = [
        'umi_create', 'umi_process',
        'umi_get_param_count',
        'umi_set_param', 'umi_get_param',
        'malloc', 'free'
    ];
